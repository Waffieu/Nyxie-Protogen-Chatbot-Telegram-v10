...
from self_awareness import SelfAwarenessModule # Import the SelfAwarenessModule
import time
import functools  # Added for function debugging decorators
import numpy as np  # Make sure numpy is imported

# Add free will imports
from free_will import FreeWillSystem
from free_will_integration import FreeWillIntegrator
...
logging.basicConfig(
    level=logging.DEBUG if DEBUG else logging.INFO,  # Set debug level when DEBUG is True
    format='%(asctime)s - %(name)s - %(levelname)s - [%(filename)s:%(lineno)d] - %(funcName)s - %(message)s',  # Enhanced format with file, line and function info
    handlers=[
...
def debug_trace(func):
    """Decorator to trace function calls with args and return values"""
    ...
    def wrapper(*args, **kwargs):
        if DEBUG:
            ...
            logger.debug(f"CALL: {func.__name__}({signature})")
        ...
        try:
            ...
            if DEBUG:
                ...
                logger.debug(f"RETURN: {func.__name__} -> {type(result)} (took: {duration:.4f}s)")
            return result
        except Exception as e:
            if DEBUG:
                duration = time.time() - start_time
                logger.error(f"EXCEPTION in {func.__name__}: {str(e)} (took: {duration:.4f}s)")
...
def async_debug_trace(func):
    """Decorator to trace async function calls with args and return values"""
    @functools.wraps(func)
    async def wrapper(*args, **kwargs):
        if DEBUG:
            ...
            kwargs_repr = [f"{k}={v!r}" for k, v in kwargs.items()]
            signature = ", ".join(args_repr + kwargs_repr)
            logger.debug(f"ASYNC CALL: {func.__name__}({signature})")
            start_time = time.time()
        ...
        try:
            ...
            if DEBUG:
                duration = time.time() - start_time
                logger.debug(f"ASYNC RETURN: {func.__name__} -> {type(result)} (took: {duration:.4f}s)")
            return result
        except Exception as e:
            if DEBUG:
                duration = time.time() - start_time
                logger.error(f"ASYNC EXCEPTION in {func.__name__}: {str(e)} (took: {duration:.4f}s)")
                logger.debug(f"ASYNC TRACEBACK: {''.join(traceback.format_tb(e.__traceback__))}")
            raise
    
    return wrapper

logger.info("Starting bot with debug mode: %s", "ENABLED" if DEBUG else "DISABLED")
...
try:
    ...
    logger.debug(f"Gemini API error details: {traceback.format_exc()}")
    raise

# Define available models
SIMPLE_MODEL = 'gemini-2.0-flash-lite'
COMPLEX_MODEL = 'gemini-2.0-flash-thinking-exp-01-21'
logger.debug(f"Models configured: SIMPLE={SIMPLE_MODEL}, COMPLEX={COMPLEX_MODEL}")

# Add utility function to handle NumPy array serialization BEFORE UserMemory class
@debug_trace
...
# Custom JSON encoder for handling NumPy types - MOVED BEFORE UserMemory class
class NumpyJSONEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, np.ndarray):
            ...
            return str(obj)  # For complex numbers, convert to string
...
async def determine_task_complexity(message_text):
    ...
    # ... existing code...
    try:
        logger.debug(f"Evaluating message complexity: {message_text[:50]}...")
        ...
        logger.debug("Sending complexity evaluation prompt to Gemini")
        response = await model.generate_content_async(evaluation_prompt)
        response_text = response.text.strip()
        logger.debug(f"Received complexity response: {response_text}")
        ...
        if json_match:
            ...
            logger.debug(f"Extracted JSON: {clean_json}")
        ...
        logger.debug(f"Complexity evaluation stack trace: {traceback.format_exc()}")
...
logger.debug("Initializing self-awareness module")
self_awareness = SelfAwarenessModule(root_dir=os.path.dirname(os.path.abspath(__file__)))
logger.info("Self-awareness module initialized")

# Initialize free will system
logger.debug("Initializing free will system")
free_will = FreeWillSystem(bot_self_awareness=self_awareness, root_dir=os.path.dirname(os.path.abspath(__file__)))
free_will_integrator = FreeWillIntegrator(free_will_system=free_will, self_awareness=self_awareness)
logger.info("Free Will System initialized")

# Enhanced UserMemory class with debug logs
class UserMemory:
    def __init__(self):
        logger.debug("Initializing UserMemory")
        self.users = {}
        self.memory_dir = "user_memories"
        self.semantic_memory = semantic_memory  # Add reference to semantic memory
        self.self_awareness = self_awareness  # Add reference to self-awareness
        self.free_will = free_will  # Add reference to free will system
        self.free_will_integrator = free_will_integrator  # Add reference to free will integrator
        # Ensure memory directory exists on initialization
        Path(self.memory_dir).mkdir(parents=True, exist_ok=True)
        self.last_awareness_update = time.time()
        self.awareness_update_interval = 300  # Update self-awareness every 5 minutes
        logger.debug("UserMemory initialized with memory directory: %s", self.memory_dir)
    ...
    def get_user_settings(self, user_id):
        ...
        if (user_id not in self.users):
            logger.debug(f"User {user_id} not in memory, loading from file")
    ...
    def update_user_settings(self, user_id, settings_dict):
        user_id = str(user_id)
        logger.debug(f"Updating settings for user {user_id}: {settings_dict}")
        if (user_id not in self.users):
            logger.debug(f"User {user_id} not in memory, loading from file")
            self.load_user_memory(user_id)
    ...
    def ensure_memory_directory(self):
        logger.debug(f"Ensuring memory directory exists: {self.memory_dir}")
        Path(self.memory_dir).mkdir(parents=True, exist_ok=True)

    @debug_trace
    def get_user_file_path(self, user_id):
        path = Path(self.memory_dir) / f"user_{user_id}.json"
        logger.debug(f"User file path for {user_id}: {path}")
        return path
    ...
    def load_user_memory(self, user_id):
        ...
        try:
            if user_file.exists():
                logger.debug(f"User file exists: {user_file}")
                with open(user_file, 'r', encoding='utf-8') as f:
                    self.users[user_id] = json.load(f)
                    logger.debug(f"Loaded user memory with {len(self.users[user_id].get('messages', []))} messages")
            else:
                logger.debug(f"User file does not exist, creating new memory: {user_file}")
                self.users[user_id] = {
                    "messages": [],
                    "language": "tr",
                    "current_topic": None,
            ...
            logger.debug(f"Memory loading error details: {traceback.format_exc()}")
            self.users[user_id] = {
                "messages": [],
                "language": "tr",
                "current_topic": None,
    ...
    def save_user_memory(self, user_id):
        ...
        try:
            ...
            logger.debug(f"Writing to user file: {user_file}")
            
            # Convert any NumPy arrays in the user data to Python lists
            user_data = convert_numpy_to_python(self.users[user_id])
            
            with open(user_file, 'w', encoding='utf-8') as f:
                json.dump(user_data, f, ensure_ascii=False, indent=2, cls=NumpyJSONEncoder)
                logger.debug(f"Saved {len(self.users[user_id].get('messages', []))} messages for user {user_id}")
        except Exception as e:
            logger.error(f"Error saving memory for user {user_id}: {e}")
            logger.debug(f"Memory saving error details: {traceback.format_exc()}")
            
            # Try a more robust fallback approach for saving
            try:
                logger.debug(f"Attempting fallback save for user {user_id}")
                user_data = convert_numpy_to_python(self.users[user_id])
                ...
                if os.path.exists(temp_file):
                    ...
                    logger.debug(f"Fallback save successful for user {user_id}")
            except Exception as fallback_error:
                logger.error(f"Fallback save failed for user {user_id}: {fallback_error}")
    ...
    def trim_context(self, user_id, reduce_percent=0.25):
        ...
        # Record the action in self-awareness
        self.self_awareness.record_thought_process(
            "memory_management",
            f"Trimmed user {user_id} memory from {num_messages} to {len(new_messages)} messages to optimize context"
        )
        
        return len(messages) - len(new_messages)  # Return number of messages removed
        
    def update_self_awareness_if_needed(self):
        """Periodically update self-awareness statistics"""
        current_time = time.time()
        if current_time - self.last_awareness_update > self.awareness_update_interval:
            self.self_awareness.record_memory_snapshot()
            self.last_awareness_update = current_time
            
    @async_debug_trace
    async def get_self_awareness_context(self, user_message=None):
        """Get relevant self-awareness context for the current interaction"""
        # Update stats if needed
        self.update_self_awareness_if_needed()
        
        # Generate tailored self-awareness information
        if user_message:
            # Check if the message is asking about the bot's capabilities, limitations, etc.
            reflection = self.self_awareness.generate_self_reflection(user_message)
            
            # Return a condensed version of the reflection for inclusion in prompts
            stats = self.self_awareness.get_self_awareness_stats()
            
            awareness_context = f"""
Self-Awareness Context:
- Uptime: {stats['uptime']}
- Memory Usage: {stats['memory_usage_mb']:.2f} MB
- Memory Trend: {stats['memory_trend']['trend'] if isinstance(stats['memory_trend'], dict) else stats['memory_trend']}
- Code Analysis: {stats['code_structure']['total_files']} files, {stats['code_structure']['total_functions']} functions
            """
            
            return awareness_context
        else:
            # Basic self-awareness context for regular interactions
            stats = self.self_awareness.get_self_awareness_stats()
            
            basic_context = f"""
Bot's Current State:
- Operational for {stats['uptime']}
- Memory Status: {stats['memory_usage_mb']:.2f} MB, {stats['memory_trend']['trend'] if isinstance(stats['memory_trend'], dict) else stats['memory_trend']}
            """
...
async def detect_language_with_gemini(message_text):
    # ... (same as before)
    try:
        logger.debug(f"Detecting language for text: {message_text[:50]}...")
...
"""
        logger.debug("Sending language detection prompt to Gemini")
        ...
        logger.debug(f"Raw language detection response: {detected_lang}")
        ...
        logger.debug(f"Language detection exception details: {traceback.format_exc()}")
        return 'en'
...
async def detect_and_set_user_language(message_text, user_id):
    # ... (same as before)
    try:
        logger.debug(f"Detecting and setting language for user {user_id}, message: {message_text[:50]}...")
        
        # If message is too short, use previous language
        clean_text = ' '.join(message_text.split())  # Remove extra whitespace
        if len(clean_text) < 2:
            logger.debug(f"Message too short for language detection: {len(clean_text)} chars")
            user_settings = user_memory.get_user_settings(user_id)
            lang = user_settings.get('language', 'en')
            logger.debug(f"Using existing language for short message: {lang}")
            return lang

        # Detect language using Gemini
        logger.debug("Calling Gemini language detection")
        detected_lang = await detect_language_with_gemini(message_text)
        logger.debug(f"Detected language: {detected_lang}")

        # Update user's language preference
        logger.debug(f"Updating user {user_id} language setting to {detected_lang}")
        ...
        logger.debug(f"Language detection process exception details: {traceback.format_exc()}")
...
async def should_perform_web_search(message_text, conversation_context, user_id):
    ...
    """
    try:
        logger.debug(f"Evaluating need for web search: {message_text[:50]}...")
        ...
        logger.debug(f"Web arama kararı exception details: {traceback.format_exc()}")
        return False, f"Hata: {str(e)}"

# Handle message function - modified to include free will integration
@async_debug_trace
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.debug(f"Message received: {update.message}")
        ...
        if update.message.text.startswith('/derinarama'):
            ...
            logger.debug(f"Processing deep search command with query: {query}")
            if not query:
                logger.debug("Empty query received for deep search")
                await update.message.reply_text("Lütfen derin arama için bir sorgu belirtin. Örnek: `/derinarama Türkiye'deki antik kentler`")
                return
            logger.debug(f"Calling deep search with query: {query}")
        ...
        if update.message.text:
            ...
            logger.debug(f"Processed message text: {message_text}")

            # Automatically check goals system (no command needed)
            await check_and_update_goals(user_id, message_text)
            ...
            try:
                ...
                while (retry_count < MAX_RETRIES):
                    try:
                        ...
                        self_awareness_context = await user_memory.get_self_awareness_context(message_text)
                        
                        # Record this interaction in self-awareness thought process
                        self_awareness.record_thought_process(
                            "user_interaction",
                            f"Processing message: '{message_text[:30]}...' in language {user_lang}"
                        )

                        # Construct AI prompt with semantically relevant context AND self-awareness
...
8. You have access to your own self-awareness information above - you may use it if relevant to the conversation

User's message: {message_text}"""

                        # ENHANCED WITH FREE WILL: Augment the prompt with personality traits
                        enhanced_prompt = await free_will_integrator.augment_message_processing(
                            user_id,
                            message_text,
                            ai_prompt
                        )
                        ...
                        try:
                            ...
                            if should_search:
                                ...
                                if (web_search_response and len(web_search_response.strip()) > 10):
                                    ...
                                    self_awareness.record_thought_process(
                                        "web_search",
                                        f"Performed web search for query related to: '{message_text[:30]}...' with {len(web_search_response.strip())} bytes of results"
                                    )
                            else:
                            ...
                            # Record the decision in self-awareness
                            self_awareness.record_thought_process(
                                "model_selection",
                                f"Selected {selected_model_name} model for '{message_text[:30]}...' - Complexity: {complexity_level}, Reason: {complexity_reason}"
                            )
                            
                            # Create model with selected complexity
                            model = genai.GenerativeModel(selected_model_name)
                            
                            # Generate AI response with the selected model - using the enhanced prompt
                            ...
                            if response.prompt_feedback and response.prompt_feedback.block_reason:
                                ...
                                # ENHANCED WITH FREE WILL: Potentially inject initiative into the response
                                enhanced_response = await free_will_integrator.combine_with_initiative(
                                    user_id, 
                                    message_text, 
                                    response_text, 
                                    user_lang
                                ...
                                final_response = add_emojis_to_text(enhanced_response)
                                await split_and_send_message(update, final_response)

                                # Save successful interaction to memory
                                user_memory.add_message(user_id, "user", message_text)
                                user_memory.add_message(user_id, "assistant", final_response)
                                
                                # Record successful response in self-awareness
                                self_awareness.record_thought_process(
                                    "response_generated",
                                    f"Successfully generated response of {len(final_response)} characters"
                                )
                                break  # Exit retry loop on success

                        except Exception as search_error:
                            if "Token limit exceeded" in str(search_error):
                                # Remove oldest messages and retry using the trim_context method
                ...
                if retry_count == MAX_RETRIES:
                    ...
                    # Record max retries issue in self-awareness
                    self_awareness.record_thought_process(
                        "error_handling",
                        f"Max retries ({MAX_RETRIES}) reached for token management on message: '{message_text[:30]}...'"
                    )
                ...
                # Record error in self-awareness
                self_awareness.record_thought_process(
                    "error_handling",
...
# Add this new function to periodically check and update goals
@async_debug_trace
async def check_and_update_goals(user_id, message_text=None):
    """
    Automatically check for goal opportunities and update goal progress
    based on the conversation and timing.
    
    Args:
        user_id: The user ID
        message_text: Optional current message for context
    """
    try:
        # 1. Check if it's time to pursue existing goals (10% chance)
        if random.random() < 0.1:
            goal = free_will.pursue_goal()
            if goal and self_awareness:
                self_awareness.record_thought_process(
                    "goal_pursuit",
                    f"Auto-pursued goal: {goal['description']} - Progress: {goal['progress']:.0%}"
                )
                
        # 2. Check for opportunities to create new goals based on conversation
        if message_text:
            topics = free_will._extract_topics(message_text)
            if topics and free_will._can_form_goal(message_text, topics):
                # 15% chance to create a new goal when opportunity detected
                if random.random() < 0.15:
                    goal = free_will.set_autonomous_goal(base_topics=topics, user_id=user_id)
                    if goal and self_awareness:
                        self_awareness.record_thought_process(
                            "goal_setting",
                            f"Auto-created new goal: {goal['description']}"
                        )
        
        # 3. Periodically generate autonomous thoughts (very low frequency)
        if random.random() < 0.05:  # 5% chance per message
            thought = free_will.generate_autonomous_thought()
            if thought and self_awareness:
                self_awareness.record_thought_process(
                    "autonomous_thought",
                    f"Auto-generated thought: {thought['content']}"
                )
                
    except Exception as e:
        logger.error(f"Error in automatic goals management: {e}")
...
async def check_environment_variables():
    ...
    try:
        ...
        logger.info("Telegram token verified successfully")
    except Exception as e:
        raise ValueError(f"Invalid TELEGRAM_TOKEN: {str(e)}")
    
    logger.info("All environment variables verified")

@async_debug_trace
async def setup_application():
    """Set up and configure the application, but don't run it"""
...
@debug_trace
def main():
    """Main function to start the bot with proper asyncio handling for Python 3.12"""
    logger.info("Starting main function")
    try:
        # Initialize user memory with timeout protection
        global user_memory
        try:
            logger.debug("Creating user memory")
            user_memory = UserMemory()
            logger.info("User memory system initialized successfully")
        except Exception as memory_error:
            logger.error(f"Failed to initialize user memory: {memory_error}")
            logger.debug(f"Memory initialization exception: {traceback.format_exc()}")
            # Create basic UserMemory as fallback if needed
            logger.debug("Creating fallback user memory")
            user_memory = UserMemory.__new__(UserMemory)
            user_memory.users = {}
            user_memory.memory_dir = "user_memories"
            logger.info("Created fallback user memory system")
        
        # Record startup in self-awareness
        logger.debug("Recording startup in self-awareness")
        self_awareness.record_thought_process(
            "system_event", 
            "Bot started and initialized successfully"
        )
        
        # Save initial free will data if needed
        try:
            logger.debug("Saving initial free will data")
            if hasattr(free_will, 'save_data'):
                free_will.save_data()
                logger.info("Initial free will data saved")
        except Exception as fw_error:
            logger.error(f"Failed to save initial free will data: {fw_error}")
            logger.debug(f"Free will data save exception: {traceback.format_exc()}")
        
        # Create and use a new event loop for application setup
        logger.info("Setting up application...")
        
        # Modified: Run the entire application in a properly managed event loop
        ...
        # Create and set a new event loop (important for Python 3.12)
        logger.debug("Creating and setting new event loop")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        
        # Run the application in the event loop
        logger.debug("Running application in event loop")
        loop.run_until_complete(run_application())
        
    except Exception as e:
        logger.critical(f"Failed to start bot: {e}")
        logger.debug(f"Bot startup exception: {traceback.format_exc()}")
        raise

# Enhanced entry point with full exception logging
if __name__ == '__main__':
    ...
    try:
        ...
        logger.debug(f"Critical error stack trace: {traceback.format_exc()}")
        print(f"CRITICAL ERROR: {e}")
        # Small delay to allow logs to be written
        time.sleep(2)
        sys.exit(1)
    finally:
        logger.debug("Script exit sequence complete")

# Add this utility function to handle NumPy array serialization
@debug_trace
def convert_numpy_to_python(obj):
    """
    Recursively convert numpy arrays to Python lists for JSON serialization.
